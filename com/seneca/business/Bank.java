...
/**
 * This class manages an ArrayList of accounts and provides methods such as
 * searching through the array through a variety of ways
 * 
...
public class Bank {
	 ...
	 */
	public synchronized Account removeAccount(String accountNumber) { // todo: Ask peter about this return type- makes
																		// better sense
		// if it were bool no?
	...
	/**
	 * Searches the bank's accounts for any accounts with the same balance as the
	 * passed parameter
	 * 
	 * @param balance the balance that needs to be searched for in the bank
	 * @return an array of Accounts containing all the accounts with the same
	 *         balance as the passed parameter
	 */
	public Account[] searchByBalance(double balance) { // @returns an ArrayList<Account> containing any accounts with
														// the same balance as the passed parameter

		ArrayList<Account> matches = new ArrayList<>(0);
		for (Account acc : m_bankAccounts) {
			if (BigDecimal.valueOf(acc.getAccountBalance()).compareTo(BigDecimal.valueOf(balance)) == 0) {
				matches.add(acc);
			}
		}
	...
	/**
	 * Searches the bank for any accounts with the same account name as the passed
	 * parameter
	 * 
	...
	/**
	 * Searches the bank for any accounts with the same account number as the passed
	 * parameter
	 * 
	...
	public String toString() {
		...
		out.append("*** Welcome to the Bank of ").append(m_bankName).append(" ***\n").append(" It has ")
				.append(m_bankAccounts.size()).append(" accounts.");

		int i = 1;
		for (Account a : m_bankAccounts) {
			out.append("\n").append(i).append(". ").append("number: ").append(a.getAccountNumber()).append(", name: ")
					.append(a.getFullName()).append(" balance: ").append(nf.format(a.getAccountBalance()));
			++i;
		}
	...
	public boolean equals(Object obj) {
		...
		if ((obj == null) || (obj.getClass() != this.getClass())) {// obj is not of same class there cannot be same
																	// object
			return false;
		}
